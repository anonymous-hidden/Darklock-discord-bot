...
class ServerBackupSystem {
    ...
    async initialize() {
        ...
        // Start auto-backup scheduler — check every 10 minutes
        this.autoBackupInterval = setInterval(() => this.runAutoBackups(), 10 * 60 * 1000);
        // Initial run 30 seconds after startup (let bot fully connect first)
        setTimeout(() => this.runAutoBackups(), 30 * 1000);

        this.bot.logger.info('ServerBackupSystem initialized (auto-backup scheduler started)');
    }

    /**
     * Auto-backup scheduler — runs every 10 min, creates backups for guilds
     * whose auto-backup interval has elapsed.
     */
    async runAutoBackups() {
        try {
            const configs = await new Promise((resolve, reject) => {
                this.db.all(
                    `SELECT * FROM backup_config WHERE auto_backup_enabled = 1`,
                    [],
            ...
            for (const config of configs) {
                try {
                    const guild = this.bot.client.guilds.cache.get(config.guild_id);
                    if (!guild) continue;

                    const intervalMs = (config.auto_backup_interval_hours || 24) * 60 * 60 * 1000;
                    const lastBackupTime = config.last_auto_backup
                        ? new Date(config.last_auto_backup).getTime()
                        : 0;

                    if (Date.now() - lastBackupTime < intervalMs) continue;

                    const result = await this.createBackup(config.guild_id, {
                        type: 'auto',
                        description: 'Scheduled automatic backup',
                        createdBy: 'system',
                        includeRoles:    config.backup_roles    !== 0,
                        includeChannels: config.backup_channels !== 0,
                        includeSettings: config.backup_settings !== 0,
                        includeBans:     config.backup_bans     === 1,
                    });

                    if (result.success) {
                        this.bot.logger.info(
                            `[ServerBackup] Auto backup created for ${guild.name} (${config.guild_id}) — ${(result.size / 1024).toFixed(1)} KB`
                        );
                    }
                } catch (guildErr) {
                    this.bot.logger.warn(
                        `[ServerBackup] Auto backup failed for guild ${config.guild_id}: ${guildErr.message}`
                    );
                }
            }
        } catch (err) {
            this.bot.logger.error('[ServerBackup] Auto-backup scheduler error:', err);
        }
    }
