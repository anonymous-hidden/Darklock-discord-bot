...
async function bootstrap(dashboard) {
    ...
    // ── Typo / legacy URL redirects ──
    app.get('/analitics', (req, res) => res.redirect(301, '/analytics' + (req.originalUrl.includes('?') ? '?' + req.originalUrl.split('?')[1] : '')));
    app.get('/analitics/*', (req, res) => res.redirect(301, req.originalUrl.replace('/analitics', '/analytics')));
