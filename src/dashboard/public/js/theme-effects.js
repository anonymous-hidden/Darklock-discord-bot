...
(function() {
    ...
    // ── Reduce Motion: kill JS animations when setting is toggled ──
    window.addEventListener('DLReduceMotion', (e) => {
        const active = e.detail?.active;
        // Kill Chart.js animations globally
        if (window.Chart) {
            window.Chart.defaults.animation = active ? false : undefined;
            window.Chart.defaults.transitions = active ? { active: { animation: { duration: 0 } } } : {};
            // Destroy and re-render any live charts to apply immediately
            if (typeof window.Chart.instances === 'object') {
                Object.values(window.Chart.instances).forEach(c => {
                    if (c && c.options) { c.options.animation = active ? false : {}; c.update('none'); }
                });
            }
        }
        // Stop particle/effect loops if active
        if (active && effectsContainer) {
            effectsContainer.innerHTML = '';
        }
    });
