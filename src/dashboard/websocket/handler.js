...
class WebSocketHandler {
    ...
    initialize(server) {
        ...
        // ── Wire up realtime analytics ──
        try {
            const AnalyticsCollector = require('../realtime/AnalyticsCollector');
            this._analyticsCollector = new AnalyticsCollector(this.bot);
            this._analyticsCollector.start();

            // On each batch flush, push to subscribed clients
            this._analyticsCollector.bus.on('flush', (guildId, events) => {
                this.broadcastToGuild(guildId, 'analytics_batch', { events });
            });

            this.bot.logger?.info('✅ Realtime analytics collector wired to WebSocket');
        } catch (err) {
            this.bot.logger?.warn('Could not start realtime analytics:', err.message);
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'analytics_subscribe':
                    this.handleAnalyticsSubscribe(ws, payload);
                    break;
                default:
    ...
    /**
     * Handle analytics subscription — sends latest snapshot immediately,
     * then relies on flush events via broadcastToGuild for live data.
     */
    async handleAnalyticsSubscribe(ws, payload) {
        const { guildId } = payload;
        if (!guildId) return this.send(ws, 'error', { message: 'Guild ID required' });
        ...
        // Also subscribe to guild events if not already
        if (!ws.subscribedGuilds.has(guildId)) {
            if (!this.guildSubscriptions.has(guildId)) {
                this.guildSubscriptions.set(guildId, new Set());
            }
            this.guildSubscriptions.get(guildId).add(ws);
        ...
        // Send latest snapshot so the client doesn't start blank
        const snapshot = this._analyticsCollector?.bus.getSnapshot(guildId);
        this.send(ws, 'analytics_snapshot', { guildId, snapshot });
        this.send(ws, 'analytics_subscribed', { guildId, status: 'live' });
    }
