...
ELEGOO_SERIAL_PORT = os.environ.get("ELEGOO_PORT", "/dev/elegoo")
BAUD_RATE = 115200
UPDATE_INTERVAL = 3
...
def is_bot_running():
    """Check bot status via status file (most reliable) then pgrep fallback."""
    # 1. Read bot_status.json — written by the bot itself every ~5 s
    try:
        if STATUS_FILE.exists():
            ...
            if ts:
                ...
                if age < 60:  # stale if older than 60 s
            ...
            try:
                ...
                if bot != last_bot:
                    if bot:
                        ...
                        send(ser, "LCD:DARKLOCK v2.0|Bot OFFLINE")
                        last_count = None  # force LCD refresh on reconnect
                    last_bot = bot
