    ...
    try:
        if STATUS_FILE.exists():
            with open(STATUS_FILE, 'r') as f:
                ...
                # Log Pi5 host info when present
                if data.get('pi5_online') and not getattr(get_guild_count_from_file, '_pi5_logged', False):
                    print(f"[Guild Display Bridge] Pi5 host '{data.get('host', 'unknown')}' is ONLINE")
                    get_guild_count_from_file._pi5_logged = True
                return count
...
def main():
    ...
    try:
        ...
        FORCE_RESEND_INTERVAL = 120  # resend every 2 minutes even if count unchanged
        ...
        while True:
            try:
                ...
                now = time.time()
                force_resend = (now - last_force_send) >= FORCE_RESEND_INTERVAL

                # Send if changed or periodic heartbeat
                if guild_count != last_count or force_resend:
                    if force_resend and guild_count == last_count:
                        pass  # heartbeat send — no print spam
                    else:
